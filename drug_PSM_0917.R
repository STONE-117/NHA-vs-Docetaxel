...
jpeg("OS_nha.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
...
title(main="FPS for NHA vs Docetaxel")
...
#"5-year Survival probability: 0.15 (95%CI 0.12 to 0.19 )"
#10year survival
summary_fit <- summary(fit, times =365.25*10)
survival_at_max_time <- summary_fit$surv[summary_fit$time == 365.25*10]
surv_ci_lower <- summary_fit$lower[summary_fit$time == 365.25*10]
surv_ci_upper <- summary_fit$upper[summary_fit$time == 365.25*10]
print(paste("10-year Survival probability:", round(survival_at_max_time,2),"(95%CI",round(surv_ci_lower,2),"to",round(surv_ci_upper,2),")"))
#"10-year Survival probability: 0.08 (95%CI 0.04 to 0.2 )"
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("OS_enz.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
...
title(main="FPS for Enzalutamide vs Docetaxel")
text(1200,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
dev.off()
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("OS_old.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2, lwd = 3 ,col=c("black","black","steelblue","steelblue"))
text(1200,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             ...
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
title(main="FPS for NHA vs Docetaxel (age>=75)")
dev.off()
#HR
sprintf("%.3f (95%%CI %.3f to %.3f)",
        exp(coef(model)["treat"]),
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("OS_young.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2, lwd = 3,col=c("black","black","steelblue","steelblue"))
text(1500,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
title(main="FPS for NHA vs Docetaxel (age<75)")
...
for (i in names(data)) {
  ...
  adjunct_IPCW.long<-survSplit(data = adjunct_IPCW,
                               cut = sort(unique(adjunct_IPCW$survtime)),
                               end = "survtime",
                               start = "Tstart",
                               event = "death",
                               id = "id")
  adjunct_IPCW.long<-adjunct_IPCW.long[order(adjunct_IPCW.long$id,adjunct_IPCW.long$survtime),]
  adjunct_IPCW.cens<-survSplit(data = adjunct_IPCW,
                               cut = sort(unique(adjunct_IPCW$survtime)),
                               end = "survtime",
                               start = "Tstart",
                               event = "censor",
                               id = "id")
  adjunct_IPCW.cens<-adjunct_IPCW.cens[order(adjunct_IPCW.cens$id,adjunct_IPCW.cens$survtime),]
  adjunct_IPCW.long$censor<-adjunct_IPCW.cens$censor
  c0<-coxph(Surv(Tstart,survtime,censor)~1,data = adjunct_IPCW.long)
  c0fit<-summary(survfit(c0),times = adjunct_IPCW.long$Tstart)
  adjunct_IPCW.long$k0ti<-c0fit$surv
  cz<-coxph(Surv(Tstart,survtime,censor)~unified_results+age_at_dispensing+disease_durtion+dx_IHD+dx_CAD+dx_PVD+px_PCI+px_coronary+dx_HF+dx_liver+dx_HT+dx_DM+dx_lipid+dx_AF+dx_arrhythmia+dx_COPD+dx_asthma+dx_alcohol+dx_smoke+dx_depression+dx_hemiplagia+dx_rheumatic+dx_renal+dx_ulcer+Rx_GnRHa+Rx_GnRHat+Rx_ARB+Rx_chemo+Rx_radiation+Rx_h2+Rx_glycoside+Rx_diuretic+Rx_AA+Rx_beta+Rx_vasodilator+Rx_aHT+Rx_alpha+Rx_RAS+Rx_CCB+Rx_anticoagulant+Rx_antiplatelet+Rx_LR+Rx_NRT+Rx_dementia+Rx_anticdiabetic+Rx_corticosteroids+Rx_NSAID+px_orchidectomy+px_prostatectomy,
            data=adjunct_IPCW.long)
  czfit<-summary(survfit(cz),times = adjunct_IPCW.long$Tstart)
  adjunct_IPCW.long$kzti<-czfit$surv
  adjunct_IPCW.long<-adjunct_IPCW.long%>% 
    mutate(ipcw=1/kzti,
           ipcw_s=k0ti/kzti)
  survival_model<-coxph(Surv(survtime,death)~treat,data=adjunct_IPCW.long,weights=ipcw_s)
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("PFS_nha.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2, lwd = 3, col=c("black","black","steelblue","steelblue"))
title(main="PSA progression-free survival for NHA vs Docetaxel")
text(1800,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
...
survival_at_max_time <- summary_fit$surv[summary_fit$time == max(matched_data$survtime)]
...
#Survival probability at the end of the study: 0.07
print(paste("95% CI for the survival probability at the end of the study: [", 
            round(surv_ci_lower,2), ",", round(surv_ci_upper,2), "]", sep = ""))
#95% CI for the survival probability at the end of the study: [0.02,0.31]
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("PFS_abi.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","abiraterone","KM Docetaxel","KM abiraterone"),lty=1:2,lwd = 3, col=c("black","black","steelblue","steelblue"))
title(main="PSA progression-free survival for Abiraterone vs Docetaxel")
text(1800,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
...
matched_data <- match.data(m.out)
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("PFS_enz.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
...
title(main="PSA progression-free survival for Enzalutamide vs Docetaxel")
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("PFS_old.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2,lwd = 3, col=c("black","black","steelblue","steelblue"))
title(main="PSA progression-free survival for NHA vs Docetaxel (age>=75)")
text(1200,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
...
love.plot(m.out, threshold = .1, which = "both")
model <- stpm2(Surv(survtime,death) ~ treat, data = matched_data)
jpeg("PFS_young.jpeg", width = 1000, height = 900,quality=1000,pointsize = 25)
plot(model, newdata=data.frame(treat=0), xlab="Time since drug dispensing (days)",lwd=2)
lines(model, newdata=data.frame(treat=1), lty=2,lwd=2)
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2, lwd=3, col=c("black","black","steelblue","steelblue"))
title(main="PSA progression-free survival for NHA vs Docetaxel (age<75)")
text(1700,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
dev.off()
