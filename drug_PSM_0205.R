...
matched_data_NHA_sup<-matched_data%>% 
  ...
  mutate(source="Overall")
#IP
wilcox.test(matched_data$IP_yearlynum_sup[matched_data$treat==1],matched_data$IP_yearlynum_sup[matched_data$treat==0])

#IP LOS
wilcox.test(matched_data$IP_yearlylos_sup[matched_data$treat==1],matched_data$IP_yearlylos_sup[matched_data$treat==0])

#AE
wilcox.test(matched_data$AE_yearlynum_sup[matched_data$treat==1],matched_data$AE_yearlynum_sup[matched_data$treat==0])

#OP
wilcox.test(matched_data$OP_yearlynum_sup[matched_data$treat==1],matched_data$OP_yearlynum_sup[matched_data$treat==0],conf.int = T)

ggplot(matched_data_NHA_sup,aes(x=treat,y=IP_yearlynum_sup))+
  ...
  geom_signif(
    comparisons = list(c("0", "1")),  # Specify the groups to compare
    map_signif_level = TRUE,          # Automatically map significance levels to symbols
    y_position = 28,                  # Adjust the position of the "**" annotation
    tip_length = 0.02                 # Adjust length of the lines
  )+
  theme_cleveland()+
  theme(
...
ggplot(matched_data_NHA_sup,aes(x=treat,y=IP_yearlylos_sup))+
  geom_boxplot(aes(fill=treat))+
  scale_fill_manual(name = "Treatment Group", 
                    values = c("0" = "blue", "1" = "red"), 
                    labels = treatment_labels) +
  labs(x="Treatment", y="Length of stay (days)", title="Inpatient Yearly Length of Stay") +
  coord_flip(ylim = c(0,200))+
  geom_signif(
    comparisons = list(c("0", "1")),  # Specify the groups to compare
    map_signif_level = TRUE,          # Automatically map significance levels to symbols
    y_position = 190,                  # Adjust the position of the "**" annotation
    tip_length = 0.02                 # Adjust length of the lines
  )+
  theme_cleveland()+
  theme(
...
ggplot(matched_data_NHA_sup,aes(x=treat,y=AE_yearlynum_sup))+
  geom_boxplot(aes(fill=treat))+
  scale_fill_manual(name = "Treatment Group", 
                    values = c("0" = "blue", "1" = "red"), 
                    labels = treatment_labels) +
  ...
  geom_signif(
    comparisons = list(c("0", "1")),  # Specify the groups to compare
    map_signif_level = TRUE,          # Automatically map significance levels to symbols
    y_position = 16,                  # Adjust the position of the "**" annotation
    tip_length = 0.02                 # Adjust length of the lines
  )+
  theme_cleveland()+
  theme(
...
ggplot(matched_data_NHA_sup,aes(x=treat,y=OP_yearlynum_sup))+
  geom_boxplot(aes(fill=treat))+
  scale_fill_manual(name = "Treatment Group", 
                    values = c("0" = "blue", "1" = "red"), 
                    labels = treatment_labels) +
  ...
  geom_signif(
    comparisons = list(c("0", "1")),  # Specify the groups to compare
    map_signif_level = TRUE,          # Automatically map significance levels to symbols
    y_position = 60,                  # Adjust the position of the "**" annotation
    tip_length = 0.02                 # Adjust length of the lines
  )+
  theme_cleveland()+
  theme(
...
lines(survfit(Surv(matched_data$survtime, matched_data$death) ~ matched_data$treat), col="steelblue", lty=1:2,lwd=4)
legend("topright", c("Docetaxel","NHA","KM Docetaxel","KM NHA"),lty=1:2,lwd=3, col=c("black","black","steelblue","steelblue"))
title(main="NHA vs Docetaxel in mHSPC")
text(1300,0.5,labels=sprintf("HR %.2f (95%%CI %.2f to %.2f )",
                             exp(coef(model)["treat"]),
                             exp(confint(model)["treat", 1]),
                             exp(confint(model)["treat", 2])))
